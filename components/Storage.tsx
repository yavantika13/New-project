...
const Storage: React.FC<StorageProps> = ({ events }) => {
    ...
    <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden shadow-2xl">
      <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-gray-900/50">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Immutable encrypted audit logs of all detected forest events.</p>
        </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gray-800/50 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="px-6 py-4 font-semibold">Data Stream</th>
            </tr>
          </thead>
            ...
            {events.length === 0 ? (
              ...
              events.map((event) => (
                <tr key={event.id} className="hover:bg-gray-800/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    {event.audioBlobUrl ? (
                      <audio controls src={event.audioBlobUrl} className="h-8 w-40 opacity-50 group-hover:opacity-100 transition-opacity" />
                    ) : (
                      <span className="text-xs text-gray-600 italic">Sensor ID: G-2401</span>
                    )}
                  </td>
                </tr>
              ))
